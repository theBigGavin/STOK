<template>
    ...
    <UDashboardPanelContent>
      ...
      <UDashboardSection title="推荐列表" description="基于多模型投票的AI选股推荐">
        <DecisionList
          ...
          @recommendation-detail="viewRecommendationDetail"
  ...
  <!-- 推荐详情模态框 -->
  <UDrawer v-model:open="showDetailModal" direction="right" size="4xl">
    <template #header>
    ...
    <template #content>
      <div v-if="selectedRecommendation" class="space-y-6">
            ...
            <div class="space-y-2">
              ...
              <div class="flex justify-between">
                ...
                <span class="text-sm font-medium">{{
                  selectedRecommendation.stock.industry || '未知'
                }}</span>
              </div>
            </div>
          </div>
                ...
                <span class="text-sm font-medium">
                  {{
                    selectedRecommendation.stock.current_price
                      ? `¥${selectedRecommendation.stock.current_price.toFixed(2)}`
                      : '未知'
                  }}
                </span>
              </div>
                ...
                >
                  {{
                    selectedRecommendation.stock.price_change_percent
                      ? `${
                          selectedRecommendation.stock.price_change_percent >= 0 ? '+' : ''
                        }${selectedRecommendation.stock.price_change_percent.toFixed(2)}%`
                      : '未知'
                  }}
                </span>
              </div>
                ...
                <span class="text-sm font-medium text-green-600">
                  {{
                    selectedRecommendation.decision.target_price
                      ? `¥${selectedRecommendation.decision.target_price.toFixed(2)}`
                      : '未设置'
                  }}
                </span>
              </div>
                ...
                <span class="text-sm font-medium text-red-600">
                  {{
                    selectedRecommendation.decision.stop_loss_price
                      ? `¥${selectedRecommendation.decision.stop_loss_price.toFixed(2)}`
                      : '未设置'
                  }}
                </span>
              </div>
        ...
        <div>
          ...
          <div class="space-y-3">
            <div
              v-for="vote in selectedRecommendation.vote_details"
              :key="vote.vote_id"
            ...
            >
              ...
              <div class="flex items-center gap-4">
                <span class="text-sm text-gray-500"
                  >置信度: {{ (vote.confidence * 100).toFixed(1) }}%</span
                >
                <span class="text-sm text-gray-500"
                  >信号强度: {{ vote.signal_strength.toFixed(2) }}</span
                >
              </div>
            </div>
          </div>
  ...
  <UDrawer
    ...
    direction="right"
    size="2xl"
    :ui="{ container: 'max-w-auto mx-2xl' }"
  >
    <template #header>
      <div class="flex items-center justify-between">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="text-center p-3 bg-gray-50 rounded-lg">
              <div class="text-lg font-bold text-blue-600">
                {{ systemStats.stocksWithDecisions }}
              </div>
  ...
  statistics.value = {
    ...
    avgConfidence:
      recommendations.value.length > 0
        ? Number(
            (
              (recommendations.value.reduce((sum, r) => sum + r.decision.confidence, 0) /
