...
export const useStockStore = defineStore('stocks', () => {
  ...
  const computedState = {
    // 活跃股票列表
    activeStocks: computed(() => state.stocks.filter(stock => stock.isActive)),
    ...
    // 最近搜索的关键词
    recentSearches: computed(() => state.searchHistory.slice(0, 10)),
  ...
  const actions = {
     ...
     * 批量获取股票数据
     */
    async fetchBatchStockData(symbols: string[], startDate: string, endDate: string) {
      state.loading = true;
      state.error = null;
