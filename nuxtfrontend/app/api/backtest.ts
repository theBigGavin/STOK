...
export const backtestApi = {
  ...
  async runBacktest(requestData: BacktestRequest): Promise<BacktestResult> {
    ...
    try {
      const response = await request<BacktestResult>('/backtest/run', {
        method: 'POST',
        body: requestData
      })
  ...
  async runBacktestAsync(requestData: BacktestRequest): Promise<BacktestProgress> {
    ...
    try {
      const response = await request<BacktestProgress>('/backtest/run-async', {
        method: 'POST',
        body: requestData
      })
  ...
  async getBacktestProgress(backtestId: string): Promise<BacktestProgress> {
    ...
    try {
      const response = await request<BacktestProgress>(`/backtest/progress/${backtestId}`, {
        method: 'GET'
      })
  ...
  async getBacktestResult(backtestId: string): Promise<BacktestResult> {
    ...
    try {
      const response = await request<BacktestResult>(`/backtest/result/${backtestId}`, {
        method: 'GET'
      })
  ...
  async getBacktestHistory(
    ...
    try {
      const response = await request<BacktestResult[]>('/backtest/history', {
        method: 'GET',
        params: {
          symbol,
  ...
  async runBatchBacktest(
    ...
    try {
      const response = await request<BacktestResult[]>('/backtest/batch', {
        method: 'POST',
        body: { requests }
      })
  ...
  async compareBacktests(backtestIds: string[]): Promise<BacktestComparison[]> {
    ...
    try {
      const response = await request<BacktestComparison[]>('/backtest/compare', {
        method: 'POST',
        body: { backtestIds }
      })
  ...
  async getBacktestStats(): Promise<BacktestStats> {
    ...
    try {
      const response = await request<BacktestStats>('/backtest/stats', {
        method: 'GET'
      })
  ...
  async deleteBacktest(backtestId: string): Promise<{ success: boolean; message: string }> {
    ...
    try {
      const response = await request<{ success: boolean; message: string }>(`/backtest/${backtestId}`, {
        method: 'DELETE'
      })
  ...
  async getBacktestTemplates(): Promise<Record<string, BacktestRequest>> {
    ...
    try {
      const response = await request<Record<string, BacktestRequest>>('/backtest/templates', {
        method: 'GET'
      })
  ...
  async saveBacktestConfig(
    ...
    try {
      const response = await request<{ success: boolean; configId: string }>('/backtest/configs', {
        method: 'POST',
        body: { name, config }
      })
...
export const useCachedBacktestApi = () => {
  ...
  return {
    ...
    async getBacktestHistory(
      ...
      return cachedGet<BacktestResult[]>('/backtest/history', {
        symbol,
        start_date: startDate,
        end_date: endDate,
        limit
     ...
     * 获取回测参数模板（带缓存）
     */
    async getBacktestTemplates(ttl: number = 24 * 60 * 60 * 1000): Promise<Record<string, BacktestRequest>> {
      const cacheKey = 'backtest-templates'
      return cachedGet<Record<string, BacktestRequest>>('/backtest/templates', undefined, cacheKey, ttl)
    },
