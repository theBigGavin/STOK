<template>
  <div>
    <!-- 错误提示 -->
    <UAlert
      v-if="decisionStore.error"
      :title="'操作失败'"
      :description="decisionStore.error"
      color="error"
      variant="solid"
      icon="i-lucide-alert-triangle"
      class="mb-4"
      closable
      @close="decisionStore.clearError()"
    />
        ...
        <UDashboardNavbar title="决策分析" :ui="{ right: 'gap-3' }">
          <template #right>
            ...
            <UButton
              ...
              variant="solid"
              icon="i-lucide-play"
              :loading="decisionStore.batchProcessing"
                ...
                <UCard>
                  <template #header>
                    <div class="flex items-center justify-between">
                      ...
                      <div class="text-sm text-muted">共 {{ filteredDecisions.length }} 条记录</div>
                    </div>
                  </template>
                    ...
                    >
                      ...
                      <template #decision-data="{ row }">
                        <div class="flex items-center gap-2">
                          <UBadge
                            :color="
                              getDecisionColor((row as DecisionResult).finalDecision.decision)
                            "
                            variant="subtle"
                            class="capitalize"
                          >
                            {{ getDecisionText((row as DecisionResult).finalDecision.decision) }}
                          </UBadge>
                        </div>
                      </template>
                      ...
                      <template #confidence-data="{ row }">
                        <div class="flex items-center gap-2">
                          <UProgress
                            :value="(row as DecisionResult).finalDecision.confidence"
                            :max="100"
                            size="sm"
                            :color="
                              getConfidenceColor((row as DecisionResult).finalDecision.confidence)
                            "
                            class="w-20"
                          />
                      ...
                      <template #actions-data="{ row }">
                        <div class="flex gap-2">
                          <UButton
                            ...
                            variant="ghost"
                            size="sm"
                            @click="viewDecisionDetails(row as DecisionResult)"
                          />
                          <UButton
                            icon="i-lucide-refresh-cw"
                            color="neutral"
                            variant="ghost"
                            size="sm"
                            @click="recalculateDecision(row as DecisionResult)"
                          />
                        </div>
                      </template>
                    </UTable>
                  ...
                  <div v-else class="space-y-6">
                    ...
                    <div>
                      ...
                      <div class="bg-elevated rounded-lg p-4">
                        ...
                        <div class="space-y-3">
                          <div
                            v-for="(count, decision) in selectedDecision.finalDecision.voteSummary"
                            :key="decision"
                    ...
                    <div>
                      ...
                      <div class="space-y-3">
                        <div
                          v-for="model in selectedModelDecisions"
                          :key="model.modelId"
                        ...
                        >
                          <div class="flex-1">
                            <div class="flex items-center justify-between mb-1">
                              ...
                              <UBadge
                                :color="getDecisionColor(model.decision)"
                                variant="subtle"
                                size="xs"
                              >
                    ...
                    <div>
                      ...
                      <div class="space-y-2">
                        ...
                        <div class="flex justify-between">
                          ...
                          <UBadge
                            :color="
                              selectedDecision.riskAssessment.isApproved ? 'success' : 'error'
                            "
                            variant="subtle"
                            size="sm"
                          >
