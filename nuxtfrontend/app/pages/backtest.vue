<template>
  <UDashboardPanel grow>
    ...
    <UDashboardPanelContent>
      ...
      <UAlert
        ...
        class="mb-6"
        @close="backtestStore.clearError()"
      />
        ...
        <div class="space-y-4">
          <div class="flex justify-between text-sm">
            <span class="text-muted">{{
              backtestStore.backtestProgress.currentStep || '正在执行回测...'
            }}</span>
            <span class="text-highlighted">{{ backtestStore.backtestProgress.progress }}%</span>
          </div>
          <UProgress
            :value="backtestStore.backtestProgress.progress"
            :max="100"
            size="lg"
            color="primary"
          />
      ...
      <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div class="space-y-4">
                ...
                <UFormGroup label="选择模型" name="modelIds">
                  <USelectMenu
                    v-model="backtestConfig.modelIds"
                ...
                <div v-if="backtestConfig.modelIds.length > 0" class="space-y-2">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <UBadge
                      v-for="modelId in backtestConfig.modelIds"
                      :key="modelId"
                      color="primary"
                      variant="subtle"
                      class="text-xs"
            ...
            <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
              ...
              <div v-if="showAdvanced" class="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
                <UFormGroup label="交易手续费" name="commission">
                  <UInput
                    v-model.number="backtestConfig.parameters.commission"
                    type="number"
                    step="0.0001"
                    placeholder="0.0003"
                    :disabled="backtestStore.loading"
                ...
                <UFormGroup label="滑点" name="slippage">
                  <UInput
                    v-model.number="backtestConfig.parameters.slippage"
                    type="number"
                    step="0.0001"
                    placeholder="0.0001"
                    :disabled="backtestStore.loading"
                ...
                <UFormGroup label="持仓限制" name="positionLimit">
                  <UInput
                    v-model.number="backtestConfig.parameters.positionLimit"
                    type="number"
                    placeholder="10"
                    :disabled="backtestStore.loading"
                  >
        ...
        <!-- 结果展示区域 -->
        <UDashboardSection
          v-if="backtestStore.currentBacktest"
          title="结果展示"
          description="回测性能指标和交易记录"
        >
        ...
        <!-- 对比分析区域 -->
        <UDashboardSection
          v-if="backtestStore.backtestResults.length > 1"
          title="对比分析"
  ...
  <!-- 对比选择模态框 -->
  <UDrawer v-model:open="showComparisonModal" direction="right">
    <template #header>
      <div class="flex items-center justify-between">
    ...
    <template #content>
      <div class="space-y-4">
        ...
        <div class="space-y-2 max-h-64 overflow-y-auto">
          <div
            v-for="result in backtestStore.backtestResults"
            :key="generateBacktestId(result)"
          ...
          >
            <div>
              ...
              <p class="text-xs text-muted">总回报: {{ (result.totalReturn * 100).toFixed(2) }}%</p>
            </div>
            <UCheckbox
              :model-value="comparisonSelection.includes(generateBacktestId(result))"
...
const generateBacktestId = (result: BacktestResult) => {
