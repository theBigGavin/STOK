    ...
    <UCard>
        ...
        <VisXYContainer v-else :data="chartData" class="h-80">
            <template v-for="model in uniqueModels" :key="model">
                <VisLine :x="x" :y="(d: PerformanceData) => getMetricValue(d, model)" :color="getModelColor(model)" />
                <VisArea :x="x" :y="(d: PerformanceData) => getMetricValue(d, model)" :color="getModelColor(model)"
                    :opacity="0.1" />
            </template>
...
const chartData = computed<PerformanceData[]>(() => {
    ...
    const dateGroups = props.data.reduce((acc, item) => {
        ...
        acc[date.toISOString()][item.modelName] = item.metrics[selectedMetric.value as keyof typeof item.metrics] || 0
        return acc
...
const tooltipTemplate = (d: PerformanceData) => {
    ...
    const lines = uniqueModels.value.map(model => {
        const value = d[model]
