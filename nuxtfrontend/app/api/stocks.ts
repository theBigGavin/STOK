...
export const stockApi = {
  ...
  async getStocks(params?: StockQueryParams): Promise<PaginatedResponse<StockInfo>> {
    ...
    try {
      const response = await request<PaginatedResponse<StockInfo>>('/stocks', {
        method: 'GET',
        params
      })
  ...
  async getStockData(
    ...
    try {
      const response = await request<StockDataResponse>(`/stocks/${symbol}/data`, {
        method: 'GET',
        params: {
  ...
  async refreshStockData(symbol: string): Promise<RefreshResponse> {
    ...
    try {
      const response = await request<RefreshResponse>(`/stocks/${symbol}/refresh`, {
        method: 'POST'
      })
  ...
  async getStockInfo(symbol: string): Promise<StockInfo> {
    ...
    try {
      const response = await request<StockInfo>(`/stocks/${symbol}`, {
        method: 'GET'
      })
  ...
  async searchStocks(query: string, limit: number = 10): Promise<StockInfo[]> {
    ...
    try {
      const response = await request<StockInfo[]>('/stocks/search', {
        method: 'GET',
        params: {
          q: query,
          limit
  ...
  async getBatchStockData(
    ...
    try {
      const response = await request<Record<string, StockDailyData[]>>('/stocks/batch/data', {
        method: 'POST',
        body: {
          symbols,
          start_date: startDate,
...
export const useCachedStockApi = () => {
  ...
  return {
     ...
     * 获取股票列表（带缓存）
     */
    async getStocks(params?: StockQueryParams, ttl: number = 5 * 60 * 1000): Promise<PaginatedResponse<StockInfo>> {
    ...
    async getStockData(
      ...
      return cachedGet<StockDataResponse>(`/stocks/${symbol}/data`, {
        start_date: startDate,
        end_date: endDate
      }, cacheKey, ttl)
    },
     ...
     * 搜索股票（带缓存）
     */
    async searchStocks(query: string, limit: number = 10, ttl: number = 2 * 60 * 1000): Promise<StockInfo[]> {
      const cacheKey = `stock-search:${query}:${limit}`
      return cachedGet<StockInfo[]>('/stocks/search', {
        q: query,
        limit
      }, cacheKey, ttl)
    },
