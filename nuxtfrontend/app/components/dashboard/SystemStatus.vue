                ...
                <div class="space-y-3">
                    <div v-for="service in serviceStatus" :key="service.name"
                        ...
                        <div class="flex items-center gap-3">
                            <UIcon :name="service.icon" class="size-5" :class="serviceStatusColor(service.status)" />
                            <div>
...
// 模拟数据（用于开发阶段）
const mockSystemHealth = computed(() => ({
    status: 'healthy' as const,
    timestamp: new Date().toISOString(),
    services: {
        api: { status: 'healthy' as const, responseTime: 23, lastChecked: new Date().toISOString() },
        database: { status: 'healthy' as const, responseTime: 45, lastChecked: new Date().toISOString() },
        redis: { status: 'healthy' as const, responseTime: 12, lastChecked: new Date().toISOString() },
        models: { status: 'healthy' as const, responseTime: 67, lastChecked: new Date().toISOString() }
    },
    system: {
        uptime: 86400, // 1天
        memoryUsage: 45.2,
...
// 开发阶段使用模拟数据
const displaySystemHealth = computed(() =>
    systemHealth.value || mockSystemHealth.value
)
</script>