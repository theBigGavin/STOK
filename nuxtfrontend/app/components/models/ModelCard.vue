      ...
      <slot name="header">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-2">
            <span class="weight-badge" :class="weightClass"> 权重: {{ model.weight }} </span>
      ...
      <slot name="content">
        ...
        <div v-if="model.performanceMetrics" class="performance-metrics">
          ...
          <div class="metrics-grid">
            <div v-if="model.performanceMetrics.accuracy !== undefined" class="metric-item">
              <span class="metric-label">准确率</span>
              <div class="metric-value">
                <span class="value"
                  >{{ (model.performanceMetrics.accuracy * 100).toFixed(1) }}%</span
                >
                <div class="metric-bar">
                  <div
                    class="metric-progress"
                    :style="{ width: `${model.performanceMetrics.accuracy * 100}%` }"
                  ></div>
                </div>
              </div>
            </div>
            <div v-if="model.performanceMetrics.totalReturn !== undefined" class="metric-item">
            ...
            <div v-if="model.performanceMetrics.sharpeRatio !== undefined" class="metric-item">
              <span class="metric-label">夏普比率</span>
              <div class="metric-value">
                <span class="value">{{ model.performanceMetrics.sharpeRatio.toFixed(2) }}</span>
              </div>
            </div>
            <div v-if="model.performanceMetrics.maxDrawdown !== undefined" class="metric-item">
              <span class="metric-label">最大回撤</span>
              <div class="metric-value">
                <span class="value negative"
                  >{{ (model.performanceMetrics.maxDrawdown * 100).toFixed(1) }}%</span
                >
              </div>
            </div>
            <div v-if="model.performanceMetrics.winRate !== undefined" class="metric-item">
              <span class="metric-label">胜率</span>
              <div class="metric-value">
                <span class="value"
                  >{{ (model.performanceMetrics.winRate * 100).toFixed(1) }}%</span
                >
                <div class="metric-bar">
                  <div
                    class="metric-progress"
                    :style="{ width: `${model.performanceMetrics.winRate * 100}%` }"
                  ></div>
                </div>
              </div>
            </div>
            <div v-if="model.performanceMetrics.precision !== undefined" class="metric-item">
              <span class="metric-label">精确率</span>
              <div class="metric-value">
                <span class="value"
                  >{{ (model.performanceMetrics.precision * 100).toFixed(1) }}%</span
                >
                <div class="metric-bar">
                  <div
                    class="metric-progress"
                    :style="{ width: `${model.performanceMetrics.precision * 100}%` }"
                  ></div>
                </div>
              </div>
            </div>
        ...
        <!-- 模型参数 -->
        <div
          v-if="model.parameters && Object.keys(model.parameters).length > 0"
    ...
    <div class="card-footer">
      <slot name="footer" :model="model">
        <div class="footer-actions">
          ...
          <button :class="statusButtonClass" @click="$emit('toggle-status', model)">
            {{ statusButtonLabel }}
          </button>
...
interface Emits {
  (e: 'view-details' | 'edit-model' | 'toggle-status' | 'retry', model: ModelInfo): void;
}
...
const statusClass = computed(() => {
  return props.model.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';
});
