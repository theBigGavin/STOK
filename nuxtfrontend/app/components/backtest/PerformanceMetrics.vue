<template>
    <div class="performance-metrics">
            ...
            <div class="metrics-actions">
                <slot name="actions">
                    <div class="flex items-center space-x-4">
                        ...
                        <button @click="$emit('export-metrics')"
        ...
        <div v-else class="metrics-container">
            ...
            <div class="detailed-stats">
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-item">
                        ...
                        <span class="stat-value positive">{{ formatCurrency(backtestResult.avgProfitPerTrade) }}</span>
                    </div>
                    <div class="stat-item">
                        <span class="stat-label">平均亏损</span>
                        <span class="stat-value negative">{{ formatCurrency(backtestResult.avgLossPerTrade) }}</span>
                    </div>
                    <div class="stat-item">
            ...
            <!-- 自定义指标插槽 -->
            <slot name="custom-metrics" :backtestResult="backtestResult"></slot>
        </div>
...
const tradingFrequency = computed(() => {
    if (!props.backtestResult || !props.backtestResult.trades || props.backtestResult.trades.length === 0) return 'N/A'

    const firstTradeDate = new Date(props.backtestResult.trades[0].date)
    const lastTradeDate = new Date(props.backtestResult.trades[props.backtestResult.trades.length - 1].date)
    const totalDays = Math.ceil((lastTradeDate.getTime() - firstTradeDate.getTime()) / (1000 * 60 * 60 * 24))
