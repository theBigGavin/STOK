    ...
    <UCard class="h-full">
        ...
        <div class="space-y-6">
            ...
            <div v-if="loading" class="flex justify-center py-12">
                <ULoadingIcon class="size-8 text-primary" />
            </div>
            ...
            <div v-else class="space-y-6">
                ...
                <div class="bg-elevated/30 rounded-lg p-4">
                    ...
                    <div class="space-y-3">
                        <div v-for="model in sortedModels" :key="model.modelName"
                            class="flex items-center justify-between">
                            <div class="flex items-center gap-3 flex-1">
                                <span class="text-sm font-medium text-highlighted min-w-24">
                                    {{ model.modelName }}
                                </span>
                                <UProgress :value="getMetricValue(model, selectedMetric)" :max="100" size="sm"
                                    :color="getProgressColor(getMetricValue(model, selectedMetric))" class="flex-1" />
                ...
                <div class="bg-elevated/30 rounded-lg p-4">
                    ...
                    <div class="h-48 flex items-end justify-between gap-2">
                        <div v-for="(trend, index) in performanceTrend" :key="index"
                            class="flex flex-col items-center flex-1">
                            <div class="w-full bg-primary/20 rounded-t transition-all duration-300 hover:bg-primary/30"
...
// 模拟数据（用于开发阶段）
const mockPerformance = computed(() => [
    {
        modelName: '技术分析模型',
        accuracy: 85.5,
        totalReturn: 12.3,
...
const displayPerformance = computed(() =>
    modelPerformance.value.length > 0 ? modelPerformance.value : mockPerformance.value
)
</script>