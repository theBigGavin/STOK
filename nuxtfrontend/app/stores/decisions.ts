...
import type { DecisionResult, ModelDecision, FinalDecision } from '~/types/decisions'
...
export const useDecisionStore = defineStore('decisions', () => {
  ...
  const computedState = {
    ...
    filteredDecisions: computed(() => {
      ...
      if (state.filters.decisionType) {
        filtered = filtered.filter(decision => 
          decision.finalDecision.decision === state.filters.decisionType
        )
      }
    ...
    highRiskDecisions: computed(() => 
      state.decisions.filter(decision => 
        decision.riskAssessment.riskLevel === 'HIGH'
      )
    ),
  ...
  const actions = {
    ...
    async fetchDecisionHistoryCached(
      ...
      try {
        const response = await cachedDecisionApi.getDecisionHistory(symbol, startDate, endDate, params)
     ...
     * 获取决策统计（带缓存）
     */
    async fetchDecisionStatsCached(
      symbol?: string,
      startDate?: string,
      endDate?: string
    ) {
