...
export const healthApi = {
  ...
  async checkHealthDetailed(): Promise<HealthCheckResponse> {
    ...
    try {
      const response = await request('/health/detailed', {
        method: 'GET',
      });
  ...
  async getPerformanceMetrics(): Promise<PerformanceMetrics> {
    ...
    try {
      const response = await request('/health/metrics', {
        method: 'GET',
      });
  ...
  async getSystemInfo(): Promise<SystemInfo> {
    ...
    try {
      const response = await request('/health/system', {
        method: 'GET',
      });
  ...
  async getConfiguration(): Promise<SystemInfo['configuration']> {
    ...
    try {
      const response = await request('/health/configuration', {
        method: 'GET',
      });
  ...
  async runPerformanceTest(): Promise<{
    ...
    try {
      const response = await request('/health/performance-test', {
        method: 'POST',
      });
...
export const useCachedHealthApi = () => {
  ...
  return {
    ...
    async getDependencies(ttl: number = 10 * 60 * 1000): Promise<SystemInfo['dependencies']> {
      ...
      return cachedGet<SystemInfo['dependencies']>(
        '/health/dependencies',
        undefined,
        cacheKey,
        ttl
    ...
    async getConfiguration(ttl: number = 10 * 60 * 1000): Promise<SystemInfo['configuration']> {
      ...
      return cachedGet<SystemInfo['configuration']>(
