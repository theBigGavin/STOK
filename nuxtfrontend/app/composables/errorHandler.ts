...
class ErrorHandler {
   ...
   * 获取用户友好的错误消息
   */
  private getFriendlyMessage(errorType: ErrorType, error?: any): string {
    const messages = {
   ...
   * 处理API错误
   */
  handleApiError(error: any, showToast: boolean = true): ApiError {
   ...
   * 处理网络错误
   */
  handleNetworkError(error: any, showToast: boolean = true): ApiError {
   ...
   * 处理超时错误
   */
  handleTimeoutError(error: any, showToast: boolean = true): ApiError {
   ...
   * 检查是否为特定类型的错误
   */
  isErrorType(error: any, errorType: ErrorType): boolean {
