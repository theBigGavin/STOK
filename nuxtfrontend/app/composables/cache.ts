...
class CacheManager {
   ...
   * 设置缓存
   */
  set<T = unknown>(key: string, value: T, ttl?: number, namespace?: string): void {
   ...
   * 获取缓存
   */
  get<T = unknown>(key: string, namespace?: string): T | null {
    const cacheKey = this.generateKey(key, namespace);
...
export const useCachedApi = () => {
   ...
   * 带缓存的GET请求
   */
  const cachedGet = async <T = unknown>(
    url: string,
    ...
    // 尝试从缓存获取
    const cached = cache.get(key, 'api') as T | null;
    if (cached) {
